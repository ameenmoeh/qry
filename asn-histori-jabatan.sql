...
SELECT TOP 100
    ...
    IIF(A.JENIS_KELAMIN=0,'Perempuan','Laki-Laki') AS JENIS_KELAMIN,
    D.STATUS_PEGAWAI, GA.JENJANG_PENDIDIKAN, HA.PANGKAT, 
    ...
    I.NO_URUT, IA.JABATAN, I.TMT_SK, IBB.MIN_TMT, DATEDIFF(YEAR,IBB.MIN_TMT,GETDATE()) AS LAMA, IA.USIA_PENSIUN, 
    CAST(
...
FROM            
    ...
    (
        SELECT NIP, KODE_SATUAN_KERJA, TMT_SK AS MIN_TMT FROM TR_JABATAN
    ) AS IBB ON IBB.NIP = A.NIP AND LEFT(IBB.KODE_SATUAN_KERJA,6) = LEFT(I.KODE_SATUAN_KERJA,6) LEFT OUTER JOIN
...
WHERE A.KODE_MUTASI=1 AND I.KODE_SATUAN_KERJA LIKE '01%'
