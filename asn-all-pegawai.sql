SELECT --TOP 10000 
	...
	E.SATUAN_KERJA AS SATKER1, D.SATUAN_KERJA AS SATKER2, A.SATUAN_KERJA, I.GRUP_SATUAN_KERJA, 
	CONVERT(VARCHAR, LEFT(RIGHT(A.NIP_BARU,10),4)+'-'+LEFT(RIGHT(A.NIP_BARU,6),2)+'-01', 23) AS MASUK_PEGAWAI,
	LEFT(A.GOL_RUANG, LEN(A.GOL_RUANG)-2) AS GRUP_GOL, DATEDIFF(YEAR, LEFT(A.NIP_BARU,8), GETDATE()) AS USIA
...
LEFT JOIN (select A.KODE_SATUAN_KERJA, B.GRUP_SATUAN_KERJA from TM_SATUAN_KERJA A LEFT JOIN TM_GRUP_SATUAN_KERJA B ON A.KODE_GRUP_SATUAN_KERJA=B.KODE_GRUP_SATUAN_KERJA) I ON I.KODE_SATUAN_KERJA=A.KODE_SATUAN_KERJA
WHERE KODE_MUTASI=1 
	...
	--AND CONVERT(VARCHAR, RIGHT(LEFT(A.NIP_BARU,12),4), 23) <> '2022'
	--AND A.KODE_SATUAN_KERJA LIKE '1102%'
order by a.KODE_SATUAN_KERJA


--SELECT * FROM TM_SATUAN_KERJA WHERE SATUAN_KERJA LIKE '%ten sleman%'