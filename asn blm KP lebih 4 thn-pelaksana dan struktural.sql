SELECT ROW_NUMBER() OVER(ORDER BY KODE_SATUAN_KERJA ASC) AS URUT, NIP_BARU, NAMA, TIPE_JABATAN, LEVEL_JABATAN, JABATAN, PANGKAT, GOL_RUANG, CAST(TMT_SK AS DATE) TMT_KP, SATKER1, SATKER2, NO_HP, KODE_SATUAN_KERJA FROM
(SELECT A.NIP, A.NIP_BARU, A.NAMA, D.KODE_TIPE_JABATAN, D1.TIPE_JABATAN, D.LEVEL_JABATAN, C.JABATAN, 
	B.KODE_SATUAN_KERJA, I.SATUAN_KERJA AS SATKER1, J.SATUAN_KERJA AS SATKER2,
	E.KODE_JENJANG_PENDIDIKAN, F.JENJANG_PENDIDIKAN, 
	G.KODE_PANGKAT, H.PANGKAT, H.GOL_RUANG, G.TMT_SK, B.TMT_SK as TMT_JAB, K.NO_HP,
	CASE 
...
WHERE 
	...
	--G.TMT_SK <> CAST('04/01/2019' AS DATE) AND
	--(YEAR(CAST( GETDATE() AS Date )) - YEAR(B.TMT_SK) > 4) AND
