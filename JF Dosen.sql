SELECT --TOP 100
    ...
    CASE 
        WHEN SF.NO_URUT>1 THEN 'Sudah Serdos' ELSE 'Belum Serdos'
    END AS SERDOS,
    CASE
        WHEN I.KODE_JABATAN='20201' THEN 'Sudah GB'
    END AS PROF,
    BS.BIDANG_STUDI,
...
FROM            
    ...
    dbo.TM_BIDANG_STUDI AS BS ON I.KODE_BIDANG_STUDI = BS.KODE_BIDANG_STUDI LEFT OUTER JOIN
    (SELECT        
    ...
    TR_SERTIFIKASI AS SF ON A.NIP = SF.NIP LEFT OUTER JOIN
    dbo.fnPengurangMK() AS l ON K.min_data = l.mk_awal AND K.max_data = l.mk_akhir 
WHERE 
    A.KODE_MUTASI=1 AND
    JB.KODE_LEVEL_JABATAN='201' 
